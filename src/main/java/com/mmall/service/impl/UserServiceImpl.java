...
public class UserServiceImpl  implements IUserService{
    ...
    public ServerResponse selectQuestion(String username) {
       ServerResponse validRepsonse=this.checkValid(username,Const.USERNAME);
...
//        String token = TokenCache.getKey(TokenCache.TOKEN_PREFIX + username);
    ...
    public ServerResponse<String> resetPassword(String passwordOld, String passwordNew, User user) {
       //防止横向越权，要校验一下这个用户的旧密码，一定要指定实这个用户，应为我们会查询一个count（1）
        //如果不指定id 那么就是true拉 count>0
    ...
    public ServerResponse<User> updateInformation(User user) {
       //username是不可以被更新的
        ...
        // 并且存在的email如果相同，不能是我们当前这个用户的
        int resultCount =userMapper.checkEmailByUserId(user.getEmail(),user.getId());
        if (resultCount>0){
            return  ServerResponse.createByErrorMessage("eamil已经存在，请更换email再尝试更新");
        }
